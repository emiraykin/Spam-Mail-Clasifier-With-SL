...
prediction1 = [1 if np.array(i)[1] > 0.45 else 0 for i in prediction_on_training_data1]
...
def lr_predict_spam(mail,dsNumber):
    if(dsNumber==1):
        ...
        prediction = model1.predict_proba(input_data_features)
        print(prediction)
...
#lr_predict_spam("let us find the right mortgage lender for you afpe  dear homeowner",1)
